...
- 使用系统本地语音或小米 MiMo 流式 TTS，跟随视频时间线播报中文。
...
Voice 是可选功能，每次打开侧边栏时都默认关闭。打开 **Settings**，在 **TTS 设置**中选择一种服务：

- **系统本地**使用 Chrome 或操作系统已经安装的中文语音。选择列表中的音色并播放测试样音，不会发送 TTS API Key。
- **小米 MiMo**使用低延迟流式模型 `mimo-v2.5-tts`。选择标准 API 或编程套餐，填写 Endpoint 和密钥，选择内置中文音色，再点击**测试并选中 MiMo**。固定中文测试样音完整播放之前，不能把 MiMo 保存为当前 TTS 服务。

修改 MiMo 关键配置后，之前的测试状态会立即失效。标准 API 默认地址为 `https://api.xiaomimimo.com/v1`，编程套餐默认使用中国区 OpenAI-compatible 地址。账号、服务可用性和价格请查看[小米 MiMo 官方语音合成文档](https://mimo.mi.com/docs/usage-guide/audio/speech-synthesis-v2.5)。
...
4. 打开 **Voice**，跟随字幕内容收听中文播报。Transcript 必须保持开启。
...
- 原文、简体中文和双语对照字幕。
- 跟随视频时间线的中文 Voice 播报，支持系统本地语音和小米 MiMo 流式 TTS。
- `0.85x` 到 `1.8x` 自适应语速、原声降低，以及播报落后两段时自动暂停视频。
...
4. 选择小米 MiMo TTS 时，Voice 只发送当前中文播报分段和语速风格指令；系统本地语音不离开设备。流式 PCM 音频仅在内存中播放，不会缓存。
...
- 先打开 **Transcript**。没有加载字幕时 Voice 不可用，而且每次打开侧边栏都会恢复为关闭。
- 使用系统本地语音时，回到 **TTS 设置**试听另一个已安装的中文音色。
- 使用小米 MiMo 时，重新检查接入版本、Endpoint、密钥、模型和音色，再完整播放一次测试样音。`401` 或 `403` 表示身份验证失败，`429` 表示触发 MiMo 限速。
- 更新项目后，在 Chrome 中重新加载扩展并刷新 YouTube 页面。关闭 Voice 或侧边栏会恢复视频原始音量，并取消 Voice 触发的暂停。
