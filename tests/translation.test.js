...
test("player caption translations merge into the matching digest cache by stable ID", () => {
  const { mergePlayerCaptionTranslations } = loadBackgroundHelpers();
  const existing = {
    transcript: [{ start: 0, text: "Source" }],
    paragraphCache: { "other:zh:semantic:segment-0-0": "Other video" },
  };

  const merged = mergePlayerCaptionTranslations(existing, "video-123", [
    { id: "segment-0-0", text: "第一句。" },
    { id: "segment-1-5000", text: "第二句。" },
    { id: "segment-2-10000", text: "", error: "Missing translation" },
  ]);

  assert.equal(
    merged.paragraphCache["video-123:zh:semantic:segment-0-0"],
    "第一句。",
  );
  assert.equal(
    merged.paragraphCache["video-123:zh:semantic:segment-1-5000"],
    "第二句。",
  );
  assert.equal(
    merged.paragraphCache["video-123:zh:semantic:segment-2-10000"],
    undefined,
  );
  assert.equal(
    merged.paragraphCache["other:zh:semantic:segment-0-0"],
    "Other video",
  );
  assert.notEqual(merged, existing);
});

test("player caption state follows the sidepanel mode and reuses cached translations", () => {
  const { buildPlayerCaptionState } = loadSidepanelHelpers();
  const state = buildPlayerCaptionState(
    "video-123",
    "Video title",
    "bilingual",
    [
      {
        start: 0,
        text: "The first complete sentence is intentionally long enough to form its own caption.",
      },
      {
        start: 4,
        text: "The second complete sentence is intentionally long enough to form its own caption.",
      },
    ],
    new Map([["video-123:zh:semantic:segment-0-0", "第一句。"]]),
  );

  assert.equal(state.action, "setPlayerCaptionState");
  assert.equal(state.mode, "bilingual");
  assert.equal(state.videoId, "video-123");
  assert.equal(state.segments.length, 2);
  assert.equal(state.translations["segment-0-0"], "第一句。");
  assert.equal(state.translations["segment-1-4000"], undefined);
});
