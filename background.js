...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "translatePlayerCaptionBatch") {
    handlePlayerCaptionBatch(
      message.videoId,
      message.content,
      message.videoTitle,
    )
...
function mergePlayerCaptionTranslations(cache, videoId, segments) {
  const safeCache = cache && typeof cache === "object" ? { ...cache } : {};
  const paragraphCache = {
    ...(safeCache.paragraphCache && typeof safeCache.paragraphCache === "object"
      ? safeCache.paragraphCache
      : {}),
  };
  for (const segment of Array.isArray(segments) ? segments : []) {
    if (
      ...
      paragraphCache[`${videoId}:zh:semantic:${segment.id}`] =
        segment.text.trim();
    }
  }
  return { ...safeCache, paragraphCache };
}

async function handlePlayerCaptionBatch(videoId, content, videoTitle) {
  ...
  const merged = mergePlayerCaptionTranslations(
    stored[cacheKey],
    videoId,
